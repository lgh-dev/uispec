#!/usr/bin/env node

/**
 * UIKit CLI - 安装器
 * 用于安装 UIKit 命令到 Claude Code 或 Qoder
 */

import { readFileSync, writeFileSync, existsSync, mkdirSync, cpSync, rmSync, readdirSync } from 'fs';
import { join, dirname } from 'path';
import { fileURLToPath } from 'url';
import { homedir } from 'os';

// 获取当前文件目录
// 对于编译后的 Bun 二进制，import.meta.url 会是 file:///$bunfs/root/uikit.js
// 对于普通运行，import.meta.url 会是实际文件路径
const __filename = fileURLToPath(import.meta.url);
const __dirname = dirname(__filename);

// 检测是否是编译后的二进制（通过检查是否在 $bunfs 虚拟文件系统中）
const isBunBinary = __dirname.startsWith('/$bunfs') || __dirname.startsWith('\\$bunfs');

// GitHub 仓库配置
const GITHUB_REPO = 'lgh-dev/uikit';
const GITHUB_BRANCH = 'main';

// 确定 UIKit 根目录和资源目录
let UIKIT_ROOT, SPECS_DIR, COMMANDS_DIR;

if (isBunBinary) {
  // 编译后的二进制文件
  UIKIT_ROOT = dirname(process.execPath);
  SPECS_DIR = join(UIKIT_ROOT, 'specs');
  COMMANDS_DIR = join(UIKIT_ROOT, 'commands');

  // 开发环境特殊处理：如果二进制在 dist/ 目录，尝试使用父目录的 specs 和 commands
  if (UIKIT_ROOT.endsWith('/dist') || UIKIT_ROOT.endsWith('\\dist')) {
    const parentDir = dirname(UIKIT_ROOT);
    const parentSpecs = join(parentDir, 'specs');
    const parentCommands = join(parentDir, 'commands');

    if (existsSync(parentSpecs)) {
      SPECS_DIR = parentSpecs;
    }
    if (existsSync(parentCommands)) {
      COMMANDS_DIR = parentCommands;
    }
  }
} else {
  // 源代码运行
  UIKIT_ROOT = join(__dirname, '..');
  SPECS_DIR = join(UIKIT_ROOT, 'specs');
  COMMANDS_DIR = join(UIKIT_ROOT, 'commands');
}

// 版本号
const VERSION = '0.0.1';

// ANSI 颜色
const colors = {
  reset: '\x1b[0m',
  red: '\x1b[31m',
  green: '\x1b[32m',
  yellow: '\x1b[33m',
  blue: '\x1b[34m',
  cyan: '\x1b[36m',
  gray: '\x1b[90m'
};

const log = {
  info: (msg) => console.log(`${colors.blue}[INFO]${colors.reset} ${msg}`),
  success: (msg) => console.log(`${colors.green}[SUCCESS]${colors.reset} ${msg}`),
  warning: (msg) => console.log(`${colors.yellow}[WARNING]${colors.reset} ${msg}`),
  error: (msg) => console.log(`${colors.red}[ERROR]${colors.reset} ${msg}`)
};

// 显示 Banner
function showBanner() {
  console.log(`
${colors.cyan}╔═══════════════════════════════════════════════╗
║                                               ║
║           UIKit CLI v${VERSION}                    ║
║         UI 规范管理和安装工具                 ║
║                                               ║
╚═══════════════════════════════════════════════╝${colors.reset}
`);
}

// 创建命令文件内容
function createCommandContent(commandName, description, implementation) {
  return `#!/usr/bin/env node

/**
 * ${commandName} - ${description}
 * Generated by UIKit CLI
 */

import { readFileSync, writeFileSync, existsSync, mkdirSync, readdirSync } from 'fs';
import { join, dirname } from 'path';
import { homedir } from 'os';

const CONFIG_FILE = join(process.cwd(), '.uikit', 'current-spec.json');
const SPECS_DIR = join(homedir(), '.uikit', 'specs');

${implementation}

// 执行主函数
main().catch(error => {
  console.error('执行失败:', error.message);
  process.exit(1);
});
`;
}

// 从 GitHub 下载文件
async function downloadFromGitHub(path) {
  const url = `https://raw.githubusercontent.com/${GITHUB_REPO}/${GITHUB_BRANCH}/${path}`;

  try {
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }
    return await response.text();
  } catch (error) {
    throw new Error(`下载失败: ${url} - ${error.message}`);
  }
}

// 从 GitHub 下载目录中的所有文件
async function downloadDirectory(dirPath, targetDir) {
  const apiUrl = `https://api.github.com/repos/${GITHUB_REPO}/contents/${dirPath}?ref=${GITHUB_BRANCH}`;

  try {
    const response = await fetch(apiUrl);
    if (!response.ok) {
      throw new Error(`HTTP ${response.status}: ${response.statusText}`);
    }

    const files = await response.json();

    // 确保目标目录存在
    if (!existsSync(targetDir)) {
      mkdirSync(targetDir, { recursive: true });
    }

    // 下载所有文件
    for (const file of files) {
      if (file.type === 'file') {
        const content = await downloadFromGitHub(`${dirPath}/${file.name}`);
        writeFileSync(join(targetDir, file.name), content);
      }
    }
  } catch (error) {
    throw new Error(`下载目录失败: ${dirPath} - ${error.message}`);
  }
}

// 初始化项目（在当前项目目录创建 .uikit 配置）
async function initProject(platform) {
  const projectDir = process.cwd();
  const uikitDir = join(projectDir, '.uikit');
  const specsDir = join(uikitDir, 'specs');
  const configFile = join(uikitDir, 'current-spec.json');

  let platformName;
  if (platform === 'claude-code') {
    platformName = 'Claude Code';
  } else if (platform === 'qoder') {
    platformName = 'Qoder';
  } else {
    log.error(`不支持的平台: ${platform}`);
    return;
  }

  log.info(`正在为 ${platformName} 初始化项目...`);

  // 创建目录结构
  if (!existsSync(uikitDir)) {
    mkdirSync(uikitDir, { recursive: true });
    log.success('✓ 创建 .uikit 目录');
  }

  if (!existsSync(specsDir)) {
    mkdirSync(specsDir, { recursive: true });
  }

  // 复制规范文件到项目目录
  // 优先从本地复制（开发环境），如果不存在则从 GitHub 下载
  if (existsSync(SPECS_DIR)) {
    cpSync(SPECS_DIR, specsDir, { recursive: true });
    log.success('✓ 复制设计规范文件到项目');
  } else {
    log.info('从 GitHub 下载设计规范文件...');
    try {
      await downloadDirectory('specs', specsDir);
      log.success('✓ 下载设计规范文件成功');
    } catch (error) {
      log.error(`下载设计规范文件失败: ${error.message}`);
      process.exit(1);
    }
  }

  // 读取命令定义文件
  // 优先从本地读取（开发环境），如果不存在则从 GitHub 下载
  let switchContent, doContent, checkContent;

  if (existsSync(COMMANDS_DIR)) {
    switchContent = readFileSync(join(COMMANDS_DIR, 'uikit-switch.md'), 'utf-8');
    doContent = readFileSync(join(COMMANDS_DIR, 'uikit-do.md'), 'utf-8');
    checkContent = readFileSync(join(COMMANDS_DIR, 'uikit-check.md'), 'utf-8');
    log.success('✓ 读取命令定义文件');
  } else {
    log.info('从 GitHub 下载命令定义文件...');
    try {
      switchContent = await downloadFromGitHub('commands/uikit-switch.md');
      doContent = await downloadFromGitHub('commands/uikit-do.md');
      checkContent = await downloadFromGitHub('commands/uikit-check.md');
      log.success('✓ 下载命令定义文件成功');
    } catch (error) {
      log.error(`下载命令定义文件失败: ${error.message}`);
      process.exit(1);
    }
  }

  // 保存初始配置文件
  const initialConfig = {
    spec: null,
    installedAt: new Date().toISOString()
  };
  writeFileSync(configFile, JSON.stringify(initialConfig, null, 2));

  // 为对应平台创建命令链接
  let platformCommandsDir;
  if (platform === 'claude-code') {
    // Claude Code 支持项目级别的命令目录
    platformCommandsDir = join(projectDir, '.claude', 'commands');
  } else if (platform === 'qoder') {
    platformCommandsDir = join(projectDir, '.qoder', 'commands');
  }

  if (platformCommandsDir) {
    if (!existsSync(platformCommandsDir)) {
      mkdirSync(platformCommandsDir, { recursive: true });
    }

    // 写入命令文件到平台命令目录
    const commands = [
      { name: 'uikit-switch', content: switchContent },
      { name: 'uikit-do', content: doContent },
      { name: 'uikit-check', content: checkContent }
    ];

    for (const cmd of commands) {
      const targetFile = join(platformCommandsDir, `${cmd.name}.md`);
      writeFileSync(targetFile, cmd.content);
      log.success(`✓ 安装命令到 ${platformName}: /${cmd.name}`);
    }
  }

  console.log(`
${colors.green}═══════════════════════════════════════════════${colors.reset}

✅ UIKit 已成功初始化到项目!

${colors.cyan}平台:${colors.reset} ${platformName}
${colors.cyan}项目目录:${colors.reset} ${projectDir}

${colors.cyan}已创建的文件:${colors.reset}
  • .uikit/specs/              - 设计规范文件
  • .uikit/current-spec.json   - 当前选中的规范
  ${platform === 'claude-code' ? '• .claude/commands/          - Claude Code 命令' : ''}
  ${platform === 'qoder' ? '• .qoder/commands/           - Qoder 命令' : ''}

${colors.cyan}可用命令:${colors.reset}
  • /uikit-switch - 选择设计规范
  • /uikit-do     - 按规范开发功能
  • /uikit-check  - 审查功能合规性

${colors.cyan}使用步骤:${colors.reset}
  1. ${colors.yellow}重启 ${platformName}${colors.reset}
  2. 输入 / 查看可用命令
  3. 使用命令开始开发

${colors.gray}提示: 规范文件在 .uikit/specs/ 目录下，可以自由修改${colors.reset}

${colors.green}═══════════════════════════════════════════════${colors.reset}
`);
}

// 卸载项目中的 UIKit
async function uninstallFromProject(platform) {
  const projectDir = process.cwd();
  const uikitDir = join(projectDir, '.uikit');

  let platformCommandsDir;
  let platformName;

  if (platform === 'claude-code') {
    platformCommandsDir = join(projectDir, '.claude', 'commands');
    platformName = 'Claude Code';
  } else if (platform === 'qoder') {
    platformCommandsDir = join(projectDir, '.qoder', 'commands');
    platformName = 'Qoder';
  } else if (platform === 'all') {
    // 卸载所有
    await uninstallFromProject('claude-code');
    await uninstallFromProject('qoder');

    // 删除整个 .uikit 目录
    if (existsSync(uikitDir)) {
      rmSync(uikitDir, { recursive: true });
      log.success('✓ 删除 .uikit 目录');
    }
    return;
  } else {
    log.error(`不支持的平台: ${platform}`);
    return;
  }

  log.info(`正在从项目中卸载 ${platformName} 的 UIKit 配置...`);

  // 删除平台命令
  if (platformCommandsDir && existsSync(platformCommandsDir)) {
    const commands = ['uikit-switch.md', 'uikit-do.md', 'uikit-check.md'];
    for (const cmd of commands) {
      const cmdPath = join(platformCommandsDir, cmd);
      if (existsSync(cmdPath)) {
        rmSync(cmdPath);
        log.success(`✓ 删除命令: /${cmd.replace('.md', '')}`);
      }
    }
  }

  log.success(`UIKit 已从 ${platformName} 卸载`);
}

// 显示状态
function showStatus() {
  const projectDir = process.cwd();
  const uikitDir = join(projectDir, '.uikit');
  const configFile = join(uikitDir, 'current-spec.json');
  const specsDir = join(uikitDir, 'specs');
  const claudeCommandsDir = join(projectDir, '.claude', 'commands');
  const qoderCommandsDir = join(projectDir, '.qoder', 'commands');
  const commands = ['uikit-switch.md', 'uikit-do.md', 'uikit-check.md'];

  console.log(`
${colors.cyan}📊 UIKit 项目状态${colors.reset}
${'─'.repeat(50)}`);

  console.log(`${colors.cyan}项目目录:${colors.reset} ${projectDir}`);

  // 检查是否初始化
  if (!existsSync(uikitDir)) {
    console.log(`\n${colors.gray}项目尚未初始化${colors.reset}`);
    console.log(`运行 ${colors.cyan}uikit init claude${colors.reset} 或 ${colors.cyan}uikit init qoder${colors.reset} 开始初始化`);
    return;
  }

  // 读取配置
  if (existsSync(configFile)) {
    try {
      const config = JSON.parse(readFileSync(configFile, 'utf-8'));
      if (config.spec) {
        console.log(`${colors.cyan}当前规范:${colors.reset} ${config.spec}`);
      } else {
        console.log(`${colors.gray}尚未选择规范${colors.reset}`);
      }
      console.log(`${colors.cyan}初始化时间:${colors.reset} ${new Date(config.installedAt).toLocaleDateString()}`);
    } catch (e) {
      // 忽略错误
    }
  }

  // Claude Code 状态
  console.log(`\n${colors.cyan}Claude Code:${colors.reset}`);
  let claudeInstalledCount = 0;
  for (const cmd of commands) {
    const cmdPath = join(claudeCommandsDir, cmd);
    const installed = existsSync(cmdPath);
    if (installed) claudeInstalledCount++;

    const cmdName = cmd.replace('.md', '');
    console.log(`  ${installed ? colors.green + '✓' : colors.gray + '○'}${colors.reset} /${cmdName} ${installed ? '(已安装)' : '(未安装)'}`);
  }

  // Qoder 状态
  console.log(`\n${colors.cyan}Qoder:${colors.reset}`);
  let qoderInstalledCount = 0;
  for (const cmd of commands) {
    const cmdPath = join(qoderCommandsDir, cmd);
    const installed = existsSync(cmdPath);
    if (installed) qoderInstalledCount++;

    const cmdName = cmd.replace('.md', '');
    console.log(`  ${installed ? colors.green + '✓' : colors.gray + '○'}${colors.reset} /${cmdName} ${installed ? '(已安装)' : '(未安装)'}`);
  }

  // 规范文件状态
  console.log(`\n${colors.cyan}设计规范:${colors.reset}`);
  if (existsSync(specsDir)) {
    const specFiles = readdirSync(specsDir).filter(f => f.endsWith('.md'));
    console.log(`  ${colors.green}✓${colors.reset} ${specFiles.length} 个规范文件`);
    specFiles.forEach(file => {
      console.log(`  ${colors.gray}  • ${file.replace('.md', '')}${colors.reset}`);
    });
  } else {
    console.log(`  ${colors.gray}○${colors.reset} 无规范文件`);
  }

  // 当前选择的规范
  const currentSpecFile = join(uikitDir, 'current-spec.json');
  if (existsSync(currentSpecFile)) {
    try {
      const currentSpec = JSON.parse(readFileSync(currentSpecFile, 'utf-8'));
      if (currentSpec.spec) {
        console.log(`\n${colors.cyan}当前规范:${colors.reset} ${currentSpec.spec}`);
      }
    } catch (e) {
      // 忽略错误
    }
  }

  console.log('─'.repeat(50));

  const totalInstalled = claudeInstalledCount + qoderInstalledCount;
  if (totalInstalled > 0) {
    console.log(`${colors.green}✅ 项目已初始化${colors.reset}`);
  } else {
    console.log(`${colors.yellow}⚠️  命令尚未安装到任何平台${colors.reset}`);
    console.log(`运行 ${colors.cyan}uikit init claude${colors.reset} 或 ${colors.cyan}uikit init qoder${colors.reset} 安装命令`);
  }
}

// 显示帮助
function showHelp() {
  console.log(`
${colors.cyan}用法:${colors.reset}
  uikit <command> [options]

${colors.cyan}命令:${colors.reset}
  init <platform>   初始化 UIKit 到指定平台
  status            查看安装状态
  uninstall         卸载命令
  help, -h          显示帮助
  -v, --version     显示版本号

${colors.cyan}初始化示例:${colors.reset}
  uikit init claude     # 初始化到 Claude Code
  uikit init qoder      # 初始化到 Qoder
  uikit init all        # 初始化到所有平台

${colors.cyan}支持的平台:${colors.reset}
  claude    Claude Code (当前支持)
  qoder     Qoder (当前支持)
  cursor    Cursor (计划支持)
  windsurf  Windsurf (计划支持)

${colors.cyan}示例:${colors.reset}
  ${colors.gray}# 查看当前状态${colors.reset}
  uikit status

  ${colors.gray}# 初始化到 Claude Code${colors.reset}
  uikit init claude

  ${colors.gray}# 初始化到 Qoder${colors.reset}
  uikit init qoder

  ${colors.gray}# 查看版本${colors.reset}
  uikit -v

${colors.cyan}版本:${colors.reset} ${VERSION}
`);
}

// 主函数
async function main() {
  const args = process.argv.slice(2);
  const command = args[0];
  const target = args[1];

  // 处理 -v, --version
  if (command === '-v' || command === '--version') {
    console.log(`UIKit CLI v${VERSION}`);
    return;
  }

  // 处理 -h, --help
  if (command === '-h' || command === '--help') {
    showBanner();
    showHelp();
    return;
  }

  // 没有命令时显示状态
  if (!command) {
    showBanner();
    showStatus();
    console.log(`\n${colors.gray}提示: 使用 'uikit -h' 查看所有命令${colors.reset}`);
    return;
  }

  switch (command) {
    case 'init':
      showBanner();

      // 如果没有指定平台，让用户选择
      if (!target) {
        console.log(`
${colors.cyan}请选择要初始化的平台:${colors.reset}
  1. Claude Code
  2. Qoder
  3. 两个都安装

${colors.gray}提示: 下次可以直接使用${colors.reset}
  ${colors.cyan}uikit init claude${colors.reset}  # 初始化到 Claude Code
  ${colors.cyan}uikit init qoder${colors.reset}   # 初始化到 Qoder
        `);

        // 使用 readline 获取用户输入
        const readline = await import('readline');
        const rl = readline.createInterface({
          input: process.stdin,
          output: process.stdout
        });

        const answer = await new Promise(resolve => {
          rl.question('请选择 (1/2/3): ', resolve);
        });
        rl.close();

        switch (answer.trim()) {
          case '1':
            await initProject('claude-code');
            break;
          case '2':
            await initProject('qoder');
            break;
          case '3':
            await initProject('claude-code');
            await initProject('qoder');
            break;
          default:
            log.error('无效选择');
            process.exit(1);
        }
      } else {
        // 如果指定了平台
        if (target === 'claude') {
          await initProject('claude-code');
        } else if (target === 'qoder') {
          await initProject('qoder');
        } else if (target === 'all') {
          await initProject('claude-code');
          await initProject('qoder');
        } else {
          log.error(`不支持的平台: ${target}`);
          console.log('支持的平台: claude, qoder, all');
        }
      }
      break;

    // 保留 install 命令向后兼容
    case 'install':
      // 重定向到 init
      const initArgs = process.argv.slice(2);
      initArgs[0] = 'init';
      process.argv = [process.argv[0], process.argv[1], ...initArgs];
      await main();
      return;

    case 'uninstall':
      showBanner();
      if (!target) {
        console.log(`
${colors.cyan}请选择要卸载的平台:${colors.reset}
  1. Claude Code
  2. Qoder
  3. 两个都卸载
        `);

        const readline = await import('readline');
        const rl = readline.createInterface({
          input: process.stdin,
          output: process.stdout
        });

        const answer = await new Promise(resolve => {
          rl.question('请选择 (1/2/3): ', resolve);
        });
        rl.close();

        switch (answer.trim()) {
          case '1':
            await uninstallFromProject('claude-code');
            break;
          case '2':
            await uninstallFromProject('qoder');
            break;
          case '3':
            await uninstallFromProject('all');
            break;
          default:
            log.error('无效选择');
        }
      } else {
        if (target === 'claude' || target === 'qoder') {
          await uninstallFromProject(target === 'claude' ? 'claude-code' : target);
        } else if (target === 'all') {
          await uninstallFromProject('all');
        } else {
          log.error(`不支持的平台: ${target}`);
        }
      }
      break;

    case 'status':
      showBanner();
      showStatus();
      break;

    case 'help':
    case '--help':
    case '-h':
      showBanner();
      showHelp();
      break;

    case 'version':
      console.log(`UIKit CLI v${VERSION}`);
      break;

    default:
      log.error(`未知命令: ${command}`);
      console.log();
      console.log(`${colors.cyan}可用命令:${colors.reset}`);
      console.log('  init [platform]  - 初始化到指定平台');
      console.log('  status          - 查看安装状态');
      console.log('  uninstall       - 卸载命令');
      console.log('  help, -h        - 显示帮助');
      console.log('  -v, --version   - 显示版本');
      console.log();
      console.log(`${colors.gray}使用 'uikit -h' 查看详细帮助${colors.reset}`);
      process.exit(1);
  }
}

// 运行
main().catch(error => {
  log.error(`执行失败: ${error.message}`);
  process.exit(1);
});