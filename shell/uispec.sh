...
show_help() {
    ...
    echo ""
    echo -e "  ${GRAY}# 从 Claude Code 卸载${NC}"
    echo "  uispec uninstall claude"
    echo ""
    echo -e "  ${GRAY}# 从所有平台卸载${NC}"
    echo "  uispec uninstall all"
    echo ""
    echo -e "  ${GRAY}# 安装/更新 UISpec CLI 工具${NC}"
    echo "  curl -fsSL https://raw.githubusercontent.com/lgh-dev/uispec/main/install.sh | bash"
    echo ""
    echo -e "  ${GRAY}# 卸载 UISpec CLI 工具${NC}"
